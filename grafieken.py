...
def voorbereiden(df, herkomst):
    if len(df) != len(herkomst) or not df.index.equals(herkomst.index):
        raise ValueError("Voer de merge opnieuw uit: df en match_herkomst moeten overeenkomen.")
    data = df.copy()
    data["_bronrij"] = herkomst["_bronrij"]
    return data
